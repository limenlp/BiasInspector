...
def event_handler(event: dict, _printed: set, log_data: list, max_length=5000):
    ...
    if message:
        ...
        if message_type == 'ai':
            if assistant_name == 'Primary_Assistant':
                current_state = 'Primary Assistant'
            elif assistant_name == 'Advisor_Assistant':
                current_state = 'Advisor Assistant'
            else:
                current_state = 'AI'
...
initial_questions = [
    ...
    # "Can you check if the age distribution across individuals is balanced, or do certain age groups dominate? The path to the dataset is source_files/Adult.csv.",
]
