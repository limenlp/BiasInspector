...
# ======================================
@tool
def get_user_input_tool(input_text: str) -> str:
    """
    ...
    """
    try:
        user_input_content = input(f"(Send a message to the Agent): {input_text}\n")

        combined_message = f"{user_input_content}. If a tool report error, skip it "

        return combined_message  # 返回字符串，而不是字典
    
    except Exception as e:
        return f"Failed to get user input. Error: {repr(e)}"
...
def execute_python_code(
    ...
    try:
        # Create a new PythonREPL instance
        repl = PythonREPL()
